...
describe('describeDeletion', () => {
  it('says the source file is not touched', () => {
    ...
    // so deleting a recording removes ailoud's copy and leaves the original.
...
describe('ailoud rm', () => {
  it('deletes the recording and ailoud copy of its audio', async () => {
    const ctx = await withOneRecording();
    const mediaFiles = () => [...ctx.memFs.files.keys()].filter((p) => p.includes('/media/'));
    expect(mediaFiles()).toHaveLength(1);

    await buildProgram(ctx).parseAsync(['node', 'ailoud', 'rm', 'ID001', '--force']);
  ...
  it('deletes nothing at all when any id is unknown', async () => {
    const ctx = await withOneRecording();
    await expect(
      buildProgram(ctx).parseAsync(['node', 'ailoud', 'rm', 'ID001', 'NOPE', '--force']),
  ...
  it('refuses without --force when there is no terminal to ask on', async () => {
    ...
    const ctx = await withOneRecording();
    await expect(buildProgram(ctx).parseAsync(['node', 'ailoud', 'rm', 'ID001'])).rejects.toThrow(
      UsageError,
    );
    expect(await ctx.store.getRecording('ID001')).not.toBeNull();
  });
...
describe('the shared consent guard, as rm uses it', () => {
  it('names deleting and --force, not installing software and --yes', async () => {
    ...
    // recordings that ailoud "needs confirmation before installing software".
    const ctx = await withOneRecording();
    const error: unknown = await buildProgram(ctx)
      .parseAsync(['node', 'ailoud', 'rm', 'ID001'])
